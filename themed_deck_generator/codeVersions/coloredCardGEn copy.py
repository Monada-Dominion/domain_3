...
class PokemonCardGenerator:
    def __init__(self):
        ...
        self.image_index = 1  # Initialize the image index to 1
    ...
    def generate_card(self, name, description, image_path, output_path):
        try:
            ...
            header_font = ImageFont.truetype("/Users/krasnomakov/Library/Fonts/PressStart2P-Regular.ttf")
...
if __name__ == "__main__":
    ...
    for image_filename in sorted(os.listdir(images_folder)):
        if image_filename.endswith(".png") or image_filename.endswith(".jpg"):
            name = names[card_generator.image_index - 1]  # Get name based on image index
            description = descriptions[card_generator.image_index - 1]  # Get description based on image index
            image_path = os.path.join(images_folder, image_filename)
            output_path = os.path.join(output_folder, f"pokemon_card_{card_generator.image_index}.png")
            card_generator.generate_card(name, description, image_path, output_path)
            card_generator.image_index += 1
